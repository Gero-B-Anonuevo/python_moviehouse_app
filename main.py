...
class RecordWindow(Toplevel):
    ...
    def update_add_movie_button(self, event):
    ...
    def check_in(self):
        self.record.set_is_finished(False)
        
        result = self.db_manager.check_in(room_id=self.room.get_id(), movies=self.record.movies)
        ...
        self.update_add_movie_button(None)

    def check_out(self):
      
        self.record.set_is_finished(True)
       
        record_id = self.db_manager.retrieve_record(self.room.get_id()).get_id()
       
        result = self.db_manager.check_out(id=record_id)
        if result:
            self.record.movies = [] 
            self.update_listbox() 
            self.movies_list = self.db_manager.retrieve_movies([])
...
class MovieHouseWindow(Tk):
    def __init__(self, database_filename):
        ...
        self.room1_button = Button(self.right_frame, text="Room 1", command=lambda:self.open_room(1))
        self.room1_button.pack(fill="both", expand=True)

        self.room2_button = Button(self.right_frame, text="Room 2", command=lambda:self.open_room(2))
        self.room2_button.pack(fill="both", expand=True)

        self.room3_button = Button(self.right_frame, text="Room 3", command=lambda:self.open_room(3))
        self.room3_button.pack(fill="both", expand=True)

        self.room4_button = Button(self.right_frame, text="Room 4", command=lambda:self.open_room(4))
        ...
        self.add_button = Button(self.button_frame_in_register, text="Add Movie", command= self.add_movie)
        ...
        self.movies_list.bind("<<ListboxSelect>>", self.update_remove_button)
    ...
    def update_remove_button(self, event):
        if self.movies_list.curselection():
